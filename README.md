...
> 你每天对着 Claude Code 好几个小时。
> 现在,屏幕最底下那行,住进了一只**看你写代码、一点点长大的小家伙**。

你敲代码,她就在状态栏里跟着忙:你修 bug,她抡起小锤追着 bug 怪打;你读文档,她翻起书;你半天没动,她趴下打个盹等你回来。写得多了她会**升级、转职**——天天 debug 的长成侦探 🔍,埋头建设的长成工匠 🔨,博览群码的长成学者 📖,攒到头顶还会冒出一顶小皇冠 👑。

累了就 `/pet 今天好累啊` 跟她唠两句,她嘴硬心软地回你一句;心情好给她 `/pet 👍`,她会点点头。攒下的金币能去**积分商城**给她买顶帽子戴上。

不弹窗、不打扰、不卖广告——只是安安静静陪你写代码的一只小宠物。

![六种转职形象](docs/girls-lineup.png)

> *写得越多,她长得越不一样*(图为可选「美少女」皮肤;开箱默认是可自由分发的 CC0 小动物,换皮肤见下方)。

每次状态栏刷新,就是她的一格小漫画——跟着你手里的活儿换表情、换场景、冒一句话:

![场景漫画](docs/scenes2-preview.png)
...
| `/pet help` | 玩法菜单 |
| `/pet 编台词` | 让 haiku 按她人设现编一批新台词,进"生成池",空闲时和预设台词混着冒(敲一次续一批) |
